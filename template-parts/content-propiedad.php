...
<article id="post-<?php the_ID(); ?>" class="flex my-5  shadow-bottom-sm">
 ...
 $mapLocation = get_field("map_location"); if (get_field('map_location')) : ?>
			...
			<p class="t-gray ">
				<?php
 if (isset($mapLocation["city"])) { echo $mapLocation["city"]; } else { echo $mapLocation["state"]; } ?></p>
	...
	<footer class="entry-footer flex-3-md flex p-2">
		<?php
 $amount = 0; if (get_field('cuartos')) : listItems('cuartos', 'fa-solid fa-people-roof'); endif; if (get_field('cocina')) : listItems('cocina', 'fa-solid fa-kitchen-set'); endif; if (get_field('banos')) : listItems('banos', 'fa-solid fa-bath'); endif; if (get_field('parqueaderos')) : listItems('parqueaderos', 'fa-solid fa-car'); endif; ?>
